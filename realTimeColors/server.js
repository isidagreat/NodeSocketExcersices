...
io.on("connection", function(socket){
	socket.emit("greeting", {msg:color});
	
	socket.on("color", function(data){
		color = data.msg;
		console.log(data);
		socket.emit("updated_color", {msg:data});
		socket.broadcast.emit("updated_color", {msg:data});

		// if(data.msg === '0'){
		// 	counter = 0;
		// 	socket.emit("updated_message", {msg:counter});
		// 	socket.broadcast.emit("updated_message", {msg:counter});
		// }
		// else {
		// counter++;
		// socket.emit("updated_message", {msg:counter});
